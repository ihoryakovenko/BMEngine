...
void ImguiIntegration::DrawLoop(const bool& IsDrawing, GuiData Data)
{
	...
	while (!glfwWindowShouldClose(window) && IsDrawing)
	{
		...
		{
			std::chrono::duration<double, std::milli> floatDuration(1.0 / 60.0);
			std::this_thread::sleep_for(floatDuration);
		}
	}
