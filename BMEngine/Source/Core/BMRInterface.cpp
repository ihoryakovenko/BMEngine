...
bool BMRInit(GLFWwindow* Window, const BMRConfig& InConfig)
{
	...
	assert(Config.ShadersCount == SHADERS_COUNT);
	...
	BMRShaderInput ShaderInputs[SHADERS_COUNT];
	...
	MainRenderpass.CreatePipelines(LogicalDevice, Extent1, ShaderInputs, SHADERS_COUNT);
...
void BMRDraw(const BMRDrawScene& Scene)
{
	...
	{
		vkCmdBindPipeline(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.Pipelines[PipelineHandles::Terrain]);
		...
		for (u32 i = 0; i < Scene.DrawTerrainEntitiesCount; ++i)
		{
			...
			const VkDescriptorSet TerrainDescriptorSetGroup[TerrainDescriptorSetGroupCount] = {
				...
				MainRenderpass.TerrainSamplerDescriptorSets[0]
			};

			vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.PipelineLayouts[PipelineHandles::Terrain],
	...
	{
		vkCmdBindPipeline(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.Pipelines[PipelineHandles::Entity]);
		...
		for (u32 i = 0; i < Scene.DrawEntitiesCount; ++i)
		{
			...
			vkCmdPushConstants(CommandBuffer, MainRenderpass.PipelineLayouts[PipelineHandles::Entity],
				VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(BMRModel), &DrawEntity->Model);

			vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.PipelineLayouts[PipelineHandles::Entity],
	...
	{
		vkCmdBindPipeline(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.Pipelines[PipelineHandles::Deferred]);

		vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, MainRenderpass.PipelineLayouts[PipelineHandles::Deferred],
