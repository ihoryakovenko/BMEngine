...
void f_debug_mem_add(void* pointer, uint size, const char* file, uint line)
{
	uint i, j;
...
void* f_debug_mem_malloc(uint size, const char* file, uint line)
{
	void* pointer;
	uint i;
	if (f_alloc_mutex != NULL)
...
void* f_debug_mem_calloc(uint count, uint size, const char* file, uint line)
{
	void* pointer;
	uint i;
	if (f_alloc_mutex != NULL)
	...
	pointer = calloc(1, totalSize + F_MEMORY_OVER_ALLOC);

	if (pointer == NULL)
	{
		printf("MEM ERROR: Calloc returns NULL when trying to allocate %u bytes with at line %u in file %s\n", totalSize, line, file);
		if (f_alloc_mutex != NULL)
			f_alloc_mutex_unlock(f_alloc_mutex);
		f_debug_mem_print(0);
		exit(0);
	...
	for (i = 0; i < totalSize + F_MEMORY_OVER_ALLOC; i++)
		((uint8*)pointer)[i] = F_MEMORY_MAGIC_NUMBER + 1;
	f_debug_mem_add(pointer, totalSize, file, line);
	if (f_alloc_mutex != NULL)
		f_alloc_mutex_unlock(f_alloc_mutex);
	return pointer;
}
...
void* f_debug_mem_realloc(void* pointer, uint size, const char* file, uint line)
{
	uint i, j, k, move;
	void* pointer2;
	if (pointer == NULL)
